...
#停用
#gene_gene:基因間的cor矩陣，gene_list:要篩的基因列表，threshold:滿足條件的基因數量>多餘少要留
...
for i in range(len(cell_UMAP_cluster)):
    if cell_UMAP_cluster['leiden'][i] == POS_cluster:
        pos_cluster_neg_cluster_cell_list.append('positive '+str(POS_cluster))
    elif cell_UMAP_cluster['leiden'][i] == NEG_cluster:
        pos_cluster_neg_cluster_cell_list.append('negitive '+str(NEG_cluster))
    else:
        pos_cluster_neg_cluster_cell_list.append('other')
